...
With the evolution of the cloud computing paradigm, particularly within the context of Infrastructure as a Service (IaaS), the need emerged to manage and provision resources in an automated manner, giving rise to the concept of Infrastructure as Code (IaC). This approach serves as an automated and faster response to operations performed under the \emph{ClickOps} model, where provisioned infrastructure is managed manually through graphical user interfaces or ad-hoc command-line executions. Thus, the \emph{Infrastructure as Code} (IaC) paradigm emerged, treating the definition and provisioning of servers and cloud services in the same way that software code is developed \cite{morris2016infrastructure}.

According to \cite{morris2016infrastructure}, the fundamental principles of IaC are based on automating the infrastructure workflow so that configurations do not rely on a team's operational memory or passive documentation; instead, the system's state and its provisioning methods are formally documented within the code itself \cite{Bollineni2022IaCDataEngineering}. In the context of data engineering, \cite{Bollineni2022IaCDataEngineering} argues that adopting IaC mitigates significant operational risks and is an essential factor in ensuring predictability in processing pipelines.

This transformation of infrastructure into code makes the architectural requirements of a given infrastructure more traceable and predictable. Furthermore, with the aid of version control tools, it becomes possible to identify changes that may or may not have introduced structural flaws or vulnerabilities.

To create the decentralized \emph{cluster} that underpins the proposed research, utilizing the IaC paradigm is imperative, and it centers on two essential properties: \textbf{idempotency} and \textbf{reproducibility}. Declarative tools such as Terraform ensure that the final state of the infrastructure in the Oracle Cloud Infrastructure (OCI) always reflects exactly what is coded, destroying or recreating nodes only when necessary. Subsequently, task-oriented automation tools such as Ansible adapt to this scenario by configuring the operating system, performing tasks related to the environment's preliminary setup across all machines, and ultimately deploying the final environment.
...
O paradigma da computação moderna divide as arquiteturas de processadores, de forma ampla, em duas principais filosofias:
 Computadores com Conjunto Complexo de Instruções (Complex Instruction Set Computer - CISC) e Computadores com Conjunto 
 Reduzido de Instruções (Reduced Instruction Set Computer - RISC). Enquanto os processadores baseados numa microarquitetura
 x86 são capazes de oferecer instruções mais complexas e que realizam operações mais longevas devido ao número de etapas, 
 a microarquitetura ARM foca em instruções mais simples e rápidas, o que resulta em maior eficiência energética e menor 
 custo computacional. Isso faz com que consigamos ver, de maneira mais clara ao programador, que a semântica das instruções
 x86 são mais largas por linha quando comparado às de ARM, em contrapartida, um programa otimizado para ARM tende a ser mais
 longo textualmente, ou seja, ocupa mais linhas \cite{hennessy2019computer}.
 %

 Conforme \cite{hennessy2019computer}, essa modelagem distinta de design, faz com que um processador ARM seja mais eficiente
 energeticamente e termicamente, de forma que ao focar em instruções mais simples, o custo por Watt de uma instrução, tende
 a ser menor quando comparado à microarquitetura x86. 
 %

 Historicamente moldado para interagir nativamente e de forma otimizada com a difusão hegemônica da arquitetura x86-64, 
 os ecossistemas de dados em larga escala eram intrinsecamente amarrados à arquitetura CISC \cite{8287751}. Neste contexto,
 a migração de cargas de processamento de componentes como HDFS, MapReduce ou Spark para um cluster baseado em nós ARM exige 
 que desenvolvedores e pesquisadores contornem potenciais barreiras de compatibilidade cruzada, adaptação de componentes 
 e limitação da amplitude do consumo de componentes em suas instalações.
%

Contudo, apesar das primitivas complexidades de compatibilidade das imagens e pacotes base, \cite{8287751} demonstra 
através de testes de performance de Big Data que o uso de processadores ARM (commodity ARM64 servers) se mostra como 
uma abordagem muito promissora no tocante à relação custo-benefício (\textit{cost-efficiency tradeoff}). Ao viabilizar 
nós mais densos, consumindo menos energia e operando a um custo de aquisição fracionário na nuvem, ecossistemas como 
Hadoop se tornam progressivamente mais viáveis de serem experimentados e implementados do ponto de vista econômico, 
especialmente em setores públicos ou na esfera de projetos acadêmicos, onde as limitações financeiras exigem soluções 
eficientes com restrição de recursos.
