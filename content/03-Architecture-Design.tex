...
nodes, hosted within a single subnet of an OCI tenancy as depicted in
Figure~\ref{fig:topology}. Each node is a \texttt{VM.Standard.A1.Flex}
instance with 1~\emph{Oracle CPU capacity unit} (OCPU; analogous to one general-purpose vCPU) on the ARMv8 architecture, 6~GB of RAM and 50GB of storage, yielding a total cluster capacity
of 4~OCPUs, 24~GB of RAM and 200GB of storage. The physical host processors for this shape are Ampere Altra Q80-30, with a maximum frequency of 3.0~GHz; each provisioned OCPU maps to dedicated core capacity on that processor family. The instances run OL9 (RHEL-compatible) on the aarch64
...
Ambari web UI, Spark History Server, and Hive \emph{Java Database Connectivity} (JDBC)). Within the subnet, all four
...
\noindent\textbf{Phase~1: Terraform provisioning.}
...
Terraform then generates a \emph{Transport Layer Security} (TLS) RSA key pair, provisions the network layer
...
phase as seen in Figure~\ref{fig:prov-terraform}.
...
\noindent\textbf{Phase~2: OS-level configuration.}
...
and applies baseline OS security rules as shown in Figure~\ref{fig:prov-os}. Ansible is
...
services and chrony for clock synchronisation based on the \emph{Network Time Protocol} (NTP) across the
...
\noindent\textbf{Phase~3: Ambari-driven cluster deployment.}
Once the Ambari infrastructure is running, a shell script on the master node
registers the application blueprint via the Ambari \emph{representational state transfer} (REST) API. The blueprint is a
\emph{JavaScript Object Notation} (JSON) document that maps each Hadoop component to its designated node role
...
factor, \emph{Yet Another Resource Negotiator} (YARN) scheduler settings, and Hive Metastore connection parameters. The
script then submits a cluster-creation request that references the registered
blueprint and a host-mapping template as demonstrated in Figure~\ref{fig:prov-cluster}.

During this phase, Ambari downloads the ODP~1.2.2.0 \emph{Red Hat Package Manager} (RPM) packages from an
...
Ambari reports a failure at any stage, the engine retries the full deployment with a 
recovery policy. If all
...
of the ODP \citep{clemlab_odp_docs_intro_1310},
managed by Apache Ambari. ODP distributes its components as native aarch64 RPM
packages for OL9, ensuring that every service in the Hadoop
...
The architecture diagram in Figure~\ref{fig:arch-default} partitions the
platform into four functional layers: ingestion, storage, processing, and
governance. The following two subsubsections group those layers into ingestion and
storage, then processing and governance, relating the deployed ODP services to the
responsibilities implied by the diagram. Table~\ref{tab:odp-versions} lists the
software versions assumed throughout this discussion.

\subsubsection{Ingestion and storage}

Apache Kafka handles real-time event streaming, decoupling
...
provenance tracking, and a visual pipeline editor. A \emph{Network File System} (NFS) Gateway provides a
POSIX-compatible mount point for batch file ingestion, allowing external systems to write data directly
into HDFS without requiring Hadoop client libraries.

HDFS provides block-based, replicated storage for raw and
...
\subsubsection{Processing and governance}

Apache YARN schedules cluster resources by allocating
...
an alternative \emph{directed acyclic graph} (DAG)-based execution engine used by Hive for optimised query
...
Apache Ranger enforces role-based access control (RBAC)
and data-masking policies across all services in the stack; its audit logs are
indexed by Infra Solr for structured search and compliance reporting.
Solr is fed whenever Ranger persists auditable events, policy checks and
allow/deny outcomes on data and service access, administrative changes to policies
and users.
Apache Atlas complements Ranger by centralising technical metadata and
end-to-end data lineage for entities registered in the Hive Metastore and other
integrated services, supporting data discovery and impact analysis while Ranger
continues to enforce fine-grained authorization and audit policy.
Apache ZooKeeper provides distributed coordination and leader election, ensuring that
...
Phoenix), concentrating cluster resources on the analytical tier. Spark is the
primary engine for in-memory and DAG-centric execution, while Hive remains
fully deployed: Hive Server~2 continues to expose the SQL interface and JDBC/ODBC
entry point, and the shared Hive Metastore holds the canonical table definitions
that both Hive and Spark use over the same HDFS data, so disk-bound Hive
workloads and interactive Spark jobs are first-class peers rather than an
afterthought. By removing the memory and CPU overhead of NiFi, HBase, and
Phoenix, this profile frees resources for those engines. Ingestion without NiFi
still proceeds in batch through Spark (or cluster file utilities) reading and
writing datasets on HDFS, and can be extended to streaming by feeding Kafka topics
that Spark consumers process; Kafka brokers therefore remain in this profile to
support that path and future streaming integration, although they were not
exercised during the benchmark. The profile was used for the evaluation reported
in Section~\ref{sec:use-case}.

The \textbf{Streaming profile} retains NiFi, Kafka, HBase, and Phoenix but
excludes the analytical engines (Spark, Hive), targeting sustained event
streaming and flow-based ingestion while keeping HBase and Phoenix for
low-latency transactional access and point look-ups alongside the Kafka backbone.
