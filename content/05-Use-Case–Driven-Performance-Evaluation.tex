...
stress-test the environment. Em referência às consultas executadas no schema em estrela da figura
\ref{fig:ssb-schema}, essas podem ser encontradas 
abaixo:
...
Conforme pode ser verificado na figura \ref{fig:query-execution-time-comparison}, como método de execução
das consultas, todas as 14 consultas foram executadas 10 vezes, para cada um dos processadores, estes, o Hive, 
Spark, e Spark com cache. No contexto do que foi explorado, é possível verificar uma significativa diferença
entre os tempos de execução quando da comparação das execuções pelo Hiv
e pelo Spark, de forma que em grande parte das ocasiões, a mínima diferença foi próxima de 200 segundos. 
%

Quando na análise das execuções em ambas ocasiões com Spark, devido à limitações de memória da infraestrutura do Data
Lake houve pouca diferença relativa entre as execuções das consultas, em função, de não ter siod possível, também
colocar em cache, através do Spark, a tabela Lineorder, a maior tabela do schema em estrela. Isso posto, para execução
das consultas com Spark, foram adotadas as execuções abaixo:
...
Como consideração final sobre esse caso de teste com o schema em estrela, conforme implicações, pricipalmente, 
de limitações de hardware, houve muito proveito da capacidade do componente Spark de consumir memória RAM, como
também, conseguir realizar alocações em disco, quando necessário. Entretanto, em função da limitação clara do hardware
usado, ficou claro mesmo com o uso de cache, e principalmente pela inviabilidade de colocar em cache a tabela lineorder, 
o desempenho do caso de teste poderia sim ser melhorado, caso houvesse mais memória RAM disponível para o Spark.


\bnote{The performance metrics, query execution times, and resource utilisation data (CPU/RAM) are 
currently being collected and will be inserted in this section. The analysis will compare the results 
of the three benchmarking phases against the initial hypothesis.}
