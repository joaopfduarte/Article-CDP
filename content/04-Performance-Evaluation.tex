...
\paragraph{Hive versus Spark.} Across all 13 queries, Spark outperformed Hive
by a mean margin of 911~seconds, with per-query differences ranging from
282~seconds (Q1.2) to 2{,}112~seconds (Q4.1). The SSB query catalogue is
...
\texttt{dwdate}; Hive averages 553~seconds per query whereas Spark averages
223~seconds, a roughly 2.5$\times$ speed-up. The second group (Q2.1--Q2.3) adds
...
(Q1.1, Q1.2, Q1.3), the cached configuration was consistently slower, adding between
51~seconds (Q1.2) and 96~seconds (Q1.1) of overhead per query. A similar penalty
...
showed improvements of 8 to 34~seconds. Averaged across all 13 queries,
